...
const AdminComments = () => {
  ...
  const handleCreateComment = async (e) => {
    ...
    try {
      ...
      const commentedById = parseInt(newCommentData.commentedById);
      if (isNaN(commentedById)) {
      ...
      const payload = {
        ...
        taskItemId: commentType === "task" ? taskItemId : null,
        projectId: commentType === "project" ? projectId : null,
        createdAt: createdAt,
      };
      ...
      if (error.response) {
        ...
        errorMessage += `: ${error.response.status} - ${serverErrorMessage}`;
  ...
  const getFilteredComments = () => {
    return comments.filter((comment) => {
      ...
      if (filterTask && comment.taskItemId !== Number(filterTask)) {
        return false;
      }

      if (filterProject && comment.projectId !== Number(filterProject)) {
        return false;
      }

      if (filterUser && comment.commentedById !== Number(filterUser)) {
        return false;
      }
      ...
      if (filterDate) {
        ...
        if (commentDate.getTime() !== filterDateObj.getTime()) {
          return false;
        }
      }
            ...
            {showCreateForm && (
              ...
              >
                ...
                <div>
                  ...
                  <select
                    value={newCommentData.commentedById}
                    onChange={(e) =>
                      setNewCommentData((prev) => ({
                        ...prev,
                        commentedById: e.target.value,
                      }))
                    }
                    className="border border-gray-300 dark:border-gray-600 p-2 rounded w-full bg-white dark:bg-gray-700"
                    required
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Select the date and time for this comment.
                  </p>
                </div>
              ...
              <div className="p-4 overflow-x-auto">
                ...
                <div className="mt-4 flex flex-col sm:flex-row justify-between items-center">
                  ...
                  {!showAll && filteredComments.length > 10 && (
                    ...
                    >
                      Show All {filteredCount} Comments
                    </button>
                  )}
