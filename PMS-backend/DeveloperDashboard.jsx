      ...
      {!isLoading && !error && (
        <>
          ...
          <div className="flex space-x-4 mb-6">
            ...
            <div className="bg-white p-4 rounded shadow-md flex-1">
              <h3 className="text-xl font-semibold mb-2">Your Project Summary</h3>
          ...
          <div className="bg-white p-4 rounded shadow-md">
            <h3 className="text-xl font-semibold mb-4">Recent Activity (Your Projects)</h3>
            {
                ...
                <ul className="divide-y divide-gray-200">
                  {activityLogs.map((log) => (
                    <li key={log._id} className="py-3">
                      <p>
                        <span className="font-semibold">{log.user?.username || 'Unknown User'}</span>
                        <span className={`ml-1 text-${getActionColor(log.action)}-600`}>
                          {getActionText(log.action, log.details)}
                        </span>
                        <span className="ml-1 font-semibold">{log.projectTitle}</span>
                        <span className="ml-2 text-gray-500 text-sm">{formatTimestamp(log.timestamp)}</span>
                      </p>
                      {log.formattedDetails && (
                        <p className="text-gray-600 text-sm mt-1">{log.formattedDetails}</p>
                      )}
                    </li>
                  ))}
                </ul>
